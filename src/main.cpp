...
int main()
{
  const int init_result = aria2_library_init();
  ...
  aria2_global_stat_t stat = aria2_get_global_stat(session);
  ...
  if (aria2_get_global_options(session, &options, &options_count) == 0) {
  ...
  const int final_result = aria2_session_final(session);
